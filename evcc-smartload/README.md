...
Ab v4.1.0 werden Fahrzeuge über eine separate `/config/vehicles.yaml` konfiguriert.
Das Format ist **identisch zur evcc.yaml** — du kannst deine Fahrzeug-Einträge direkt kopieren.

1. `vehicles.yaml.example` nach `/config/vehicles.yaml` kopieren (HA File Editor)
2. Einträge aus deiner `evcc.yaml` einfügen
...
|----------|--------------|
| `/health` | Health-Check (`{"status": "ok", "version": "4.1.0"}`) |
