...
class WebServer:
        ...
        class Handler(BaseHTTPRequestHandler):
            ...
            def do_GET(self):
                ...
                # v6: SSE endpoint
                    ...
                    self._sse_stream()
                # Phase 6: plan timeline endpoint
                elif path == "/plan":
    ...
    def _api_plan(self, plan) -> dict:
        """Phase 6: Serialize full PlanHorizon with per-slot explanations.

        Returns a dict suitable for GET /plan, containing all 96 slots with
        German-language short and long explanations generated by ExplanationGenerator.
        """
        slots = []
        for slot in plan.slots:
            explanation = self.explanation_gen.explain(slot, plan)
            slots.append({
                "t": slot.slot_index,
