...
class Config:
    ...
    vehicle_poll_interval_minutes: int = 15  # Wie oft Fahrzeug-APIs pollen (reduziert von 30 auf 15 für aktuellere Daten)
...
def load_version() -> str:
    """Lädt Version dynamisch aus config.yaml - tries multiple locations"""
    try:
        import yaml
        # Try multiple paths in order of preference
        paths = [
            Path("/app/config.yaml"),                      # Primary: copied by Dockerfile
            Path("/etc/evcc-smartload/config.yaml"),       # Secondary: backup location
            Path("/data/config.yaml"),                     # Tertiary: data mount
            Path(__file__).parent.parent.parent / "config.yaml"  # Dev fallback
        ]
        ...
        for config_path in paths:
            if config_path.exists():
                with open(config_path, 'r') as f:
                    ...
                    version = cfg.get('version', '3.0.6')
                    _log("info", f"✓ Loaded version {version} from {config_path}")
                    return version
        
        # No config found
        _log("warning", "No config.yaml found in any location, using default version")
        return "3.0.6"  # ✅ NEW FALLBACK!
    except Exception as e:
        _log("error", f"Error loading version from config.yaml: {e}")
        return "3.0.6"  # ✅ NEW FALLBACK!
...
class VehicleStatus:
    ...
    charging: bool = False
    data_source: str = "evcc"  # 'evcc', 'direct_api', 'cache', 'unknown', 'manual'
    provider_type: str = "evcc"  # 'kia', 'renault', 'evcc', 'manual'
    manual_soc: Optional[float] = None  # Manuell eingegebener SoC
    manual_soc_timestamp: Optional[datetime] = None  # Wann wurde manuell eingegeben
    
    def is_data_stale(self, threshold_minutes: int = 60) -> bool:
        """Prüft ob Daten zu alt sind."""
    ...
    def get_data_age_string(self) -> str:
        """Gibt Alter der Daten als String zurück."""
        if not self.last_update:
            return "unbekannt"
        age = datetime.now(timezone.utc) - self.last_update
    ...
    def get_effective_soc(self) -> float:
        """Gibt effektiven SoC zurück (bevorzugt manuell wenn neuer)."""
        if self.manual_soc is not None and self.manual_soc_timestamp:
            # Prüfe ob manueller SoC neuer ist als API-Daten
...
<body>
    ...
    <!-- Modal für Manuellen SoC Input -->
    <div id="manualSocModal" style="display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.8); z-index: 1000; align-items: center; justify-content: center;">
        <div style="background: #16213e; padding: 30px; border-radius: 12px; max-width: 400px; width: 90%;">
            <h2 style="margin-top: 0; color: #00d4ff;">✏️ Manuellen SoC eingeben</h2>
            <p style="color: #888; margin-bottom: 20px;">Für: <strong id="modalVehicleName" style="color: #00ff88;"></strong></p>
            
            <div style="margin: 20px 0;">
                <label style="display: block; margin-bottom: 8px; color: #eee;">Aktueller Ladestand (%):</label>
                <input type="number" id="manualSocInput" min="0" max="100" step="1" 
                       style="width: 100%; padding: 10px; background: #0f3460; border: 1px solid #00d4ff; color: #eee; border-radius: 4px; font-size: 1.1em;"
                       placeholder="z.B. 45">
                <p style="font-size: 0.85em; color: #888; margin-top: 5px;">
                    Kapazität: <span id="modalCapacity"></span> kWh
                </p>
            </div>
            
            <div style="display: flex; gap: 10px; margin-top: 25px;">
                <button onclick="closeManualSocModal()" 
                        style="flex: 1; padding: 10px; background: #1a1a2e; color: #888; border: 1px solid #333; border-radius: 4px; cursor: pointer;">
                    Abbrechen
                </button>
                <button onclick="submitManualSoc()" 
                        style="flex: 1; padding: 10px; background: #00d4ff; color: #000; border: none; border-radius: 4px; cursor: pointer; font-weight: bold;">
                    Speichern
                </button>
            </div>
            
            <div id="modalError" style="display: none; margin-top: 15px; padding: 10px; background: #2e0a0a; border-left: 3px solid #ff4444; color: #ff4444; border-radius: 4px;"></div>
            <div id="modalSuccess" style="display: none; margin-top: 15px; padding: 10px; background: #0a2e1a; border-left: 3px solid #00ff88; color: #00ff88; border-radius: 4px;"></div>
        </div>
    </div>
    ...
    <script>
        ...
        function openManualSocModal(vehicleId, vehicleName, capacity) {{
            ...
            document.getElementById('manualSocInput').value = '';
    ...
    def _generate_device_html(self, device: dict, device_id: str) -> str:
        ...
        # Berechne "Alter" der Information & Stale Detection
        age_text = ""
        is_stale = False
        age_minutes = 0
        if last_update_str:
            try:
                ...
                # Daten sind "stale" wenn > 60 Minuten alt UND SoC > 0
                is_stale = age_seconds > 3600 and soc > 0
                
                if age_seconds < 120:  # < 2 Minuten
                    age_text = f"<span style='color: #00ff88;'>🕐 vor {age_minutes}min</span>"
                elif age_seconds < 3600:  # < 1 Stunde
                    age_text = f"<span style='color: #ffaa00;'>🕐 vor {age_minutes}min</span>"
                else:  # > 1 Stunde
        ...
        # Check ob Manual Input verfügbar (z.B. für ORA ohne API)
        show_manual_button = (soc == 0 and not slots) or device_id.startswith("ora")
        
        html = f'''
        <div class="device-card" id="device-{device_id}">
        ...
        # ⚠️ VERALTETE DATEN WARNUNG (wenn > 60min)
        if is_stale:
            html += f'''
            <div style="background: #2e2a0a; border-left: 3px solid #ffaa00; padding: 8px 12px; margin: 10px 0; border-radius: 4px;">
                <strong style="color: #ffaa00;">⚠️ Daten möglicherweise veraltet</strong><br>
                <span style="font-size: 0.85em; color: #888;">
                    Letztes Update vor {age_minutes // 60}h {age_minutes % 60}min. 
                    {f"Fahrzeug könnte unterwegs sein." if "EV" in device_id or "Twingo" in name or "ora" in device_id else ""}
                </span>
            </div>
            '''
        ...
        # ✏️ MANUELLER SOC INPUT BUTTON (für Fahrzeuge ohne API)
        if show_manual_button:
            html += f'''
            <div style="margin-top: 10px;">
                <button onclick="openManualSocModal('{device_id}', '{name}', {capacity})" 
                        style="background: #0f3460; color: #00d4ff; border: 1px solid #00d4ff; padding: 8px 16px; border-radius: 4px; cursor: pointer; font-size: 0.9em;">
                    ✏️ SoC manuell eingeben
                </button>
    ...
    def start(self):
        ...
        class Handler(BaseHTTPRequestHandler):
            ...
            def do_POST(self):
                ...
                elif self.path == '/vehicles/manual-soc':
                    # Manueller SoC Input für Fahrzeuge ohne API (v3.0.8)
                    try:
                        length = int(self.headers.get('Content-Length', 0))
                        body = json.loads(self.rfile.read(length).decode())
                        
                        vehicle_name = body.get('vehicle')
                        soc = body.get('soc')
                        
                        if not vehicle_name:
                            self._send_json({"error": "Missing vehicle name"}, 400)
                            return
                        
                        if soc is None or not (0 <= soc <= 100):
                            self._send_json({"error": "SoC must be between 0 and 100"}, 400)
                            return
                        
                        # Update Vehicle Status mit manuellem SoC
                        vehicles = api.vehicle_monitor.get_all_vehicles()
                        
                        if vehicle_name not in vehicles:
                            self._send_json({"error": f"Vehicle '{vehicle_name}' not found"}, 404)
                            return
                        
                        vehicle = vehicles[vehicle_name]
                        vehicle.manual_soc = float(soc)
                        vehicle.manual_soc_timestamp = datetime.now(timezone.utc)
                        vehicle.data_source = "manual"
                        
                        # Update im Monitor
                        api.vehicle_monitor.vehicles[vehicle_name] = vehicle
                        
                        _log("info", f"✏️ {vehicle_name}: Manual SoC set to {soc}%")
                        
                        self._send_json({
                            "status": "ok",
                            "vehicle": vehicle_name,
                            "soc": soc,
                            "timestamp": vehicle.manual_soc_timestamp.isoformat(),
                            "message": f"Manual SoC for {vehicle_name} set to {soc}%"
                        })
                    except Exception as e:
                        _log("error", f"Manual SoC error: {e}")
                        self._send_json({"error": str(e)}, 500)
