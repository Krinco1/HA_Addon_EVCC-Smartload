"""
SmartPrice v2.0 - Hybrid Optimization with Shadow Reinforcement Learning
...
<html>
<head>
    <title>SmartPrice Dashboard</title>
    <meta charset="utf-8">
            ...
            def do_GET(self):
                if self.path == '/health':
                    ...
                    self._send_json({"error": "not found", "endpoints": [
                        "/", "/health", "/status", "/summary", "/comparisons", "/config", "/vehicles"
                    ]}, 404)
...
def main():
    ...
    _log("info", "  Price Corridor: Dynamisch basierend auf Bedarf")
    _log("info", "=" * 70)
    ...
    api_server = SimpleAPIServer(cfg, optimizer, rl_agent, comparator, event_detector, collector, vehicle_monitor)
    api_server.start()
    ...
    while True:
        try:
            ...
            # === APPLY LP ACTION (only LP writes to evcc!) ===
            actual_cost = controller.apply(lp_action)
